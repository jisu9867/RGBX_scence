...
class SegEvaluator(Evaluator):
    ...
    def func_per_iteration(self, data, device):
        ...
        label = data['label']
        name = data['fn']
        pred_vpd = data['pred_vpd']
        pred_vpd = torch.from_numpy(pred_vpd).cuda()
        pred_zoe = data['pred_zoe']
        pred_zoe = torch.from_numpy(pred_zoe).cuda()
        pred = self.sliding_eval(img, config.eval_crop_size, config.eval_stride_rate, pred_vpd, pred_zoe, device)
