...
class Evaluator(object):
    ...
    def sliding_eval(self, img, crop_size, stride_rate, pred_vpd=None, pred_zoe=None, device=None):
        ...
        for s in self.multi_scales:
            ...
            processed_pred += self.scale_process(img_scale,
                                                 (ori_rows, ori_cols),
                                                 crop_size, stride_rate, pred_vpd, pred_zoe, device)

        pred = processed_pred.argmax(2)

        return pred

    def scale_process(self, img, ori_shape, crop_size, stride_rate, pred_vpd, pred_zoe,
                      device=None):
        new_rows, new_cols, c = img.shape
        long_size = new_cols if new_cols > new_rows else new_rows
        # print(long_size, "long_Size")
        # if long_size <= crop_size[1]:
        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            input_data, margin = self.process_image(img, crop_size)
            score = self.val_func_process(input_data, pred_vpd, pred_zoe, device)
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    temp_score = self.val_func_process(input_data, pred_vpd, pred_zoe, device)
    ...
    def val_func_process(self, input_data, pred_vpd, pred_zoe, device=None):
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                _, score = self.val_func(input_data, pred_vpd=pred_vpd, pred_zoe= pred_zoe)
                score = score[0]

                if self.is_flip:
                    input_data = input_data.flip(-1)
                    _, score_flip = self.val_func(input_data, pred_vpd=pred_vpd, pred_zoe= pred_zoe)
    ...
    def sliding_eval_rgbX(self, img, modal_x, crop_size, stride_rate, device=None):
        ...
        pred1 = softmax(processed_pred, axis=2)
        pred = processed_pred.argmax(2)
        return pred
    ...
    def scale_process_rgbX(self, img, modal_x, ori_shape, crop_size, stride_rate, device=None):
        ...
        if new_cols <= crop_size[1] or new_rows <= crop_size[0]:
            input_data, input_modal_x, margin = self.process_image_rgbX(img, modal_x, crop_size) #normalize, padding img print
            score = self.val_func_process_rgbX(input_data, input_modal_x, device)   # model(img, hha)
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    input_data, input_modal_x, tmargin = self.process_image_rgbX(img_sub, modal_x_sub, crop_size) #normalize, padding
    ...
    def val_func_process_rgbX(self, input_data, input_modal_x, device=None):
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                ...
                # print("input_modal_x", input_modal_x.shape)
                # print("score: ", score.shape) 1 3 480 640
                # print(score)
                score = score[0]
                if self.is_flip:
                    input_data = input_data.flip(-1)
